...
REPORTS := ${shell find wikipathways-database/pathways/ -name "WP*gpml" | sort | uniq | cut -d'/' -f4 | sed -e 's/\(WP[^.]*\).*/reports\/\1.md/' }
NEWREPORTS := ${shell find wikipathways-database/pathways/ -name "WP*gpml" | sort | uniq | cut -d'/' -f4 | sed -e 's/\(WP[^.]*\).*/reports\/\1.ng.md/' }
